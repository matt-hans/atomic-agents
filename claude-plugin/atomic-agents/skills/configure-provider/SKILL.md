...
description: Wire an Atomic Agents `AgentConfig` to a specific LLM provider — the `instructor.from_*` factory, the matching Instructor `Mode`, required `model_api_parameters`, env-var keys, and keeping `AgentConfig.mode` in sync. Use when the user asks to "switch provider", "use Anthropic/Gemini/Groq/Ollama instead", "wire up <provider>", "configure the LLM client", "change the model", or runs `/atomic-agents:configure-provider`.
---
...
For the full provider matrix — every factory, mode, role, example model, and installation extra — the authority is `../framework/references/providers.md`. This skill is the action-oriented path: clarify → plan → implement → verify → hand off.
...
- **`framework` skill**: questions about Atomic Agents in general, or work other than provider wiring.
...
For deeper material — the full provider matrix, `model_api_parameters` per provider, model-picking guidance, and installation extras — load `../framework/references/providers.md`.
