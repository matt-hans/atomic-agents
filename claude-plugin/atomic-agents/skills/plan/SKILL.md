---
name: plan
description: Turn an agent idea into a tracked Azure DevOps backlog — Epic (application) → Feature (one AtomicAgent) → User Stories (one per component) with a standardized run-anywhere Definition of Done, previewed before anything is written. Use when the user asks to "plan an agent", "create ADO work items", "generate a backlog", "break this agent into stories", "file Features/User Stories for this agent", or runs `/atomic-agents:plan`.
---
...
The full taxonomy, field/state rules, and idempotency recipe live in `../framework/references/ado-planning.md`. The run-anywhere acceptance criteria live in `../framework/references/portability-acs.md` and the gate in `../framework/references/definition-of-done.md`. Read those for depth; this skill is the action path: intake → introspect → preview → create.
...
- Capture the real field reference names (story points, acceptance criteria) and the allowed **state categories** into `.atomic/config.yml` (see `planning/config.example.yml`). Static field maps are fallback only — an unknown field **fails with remediation** (`/atomic-agents:plan --bind`), never silently emits to a stock field name.
...
3. **Provider** (default `openai`; provider is swappable later via `/atomic-agents:configure-provider`).
...
Each Story names the skill that builds it. The developer runs that skill (e.g. `/atomic-agents:create-atomic-tool`), which reads `.atomic/manifest.yaml`, pre-fills the component, and moves the Story to an in-progress state. `/atomic-agents:sync` links the branch/PR back to the Story.
...
For the taxonomy, field-map, and idempotency depth, load `../framework/references/ado-planning.md`.
