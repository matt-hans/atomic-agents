---
name: add-hooks
description: Instrument an `AtomicAgent` with Instructor hooks — telemetry, validation-error inspection, retries, and logging via `register_hook`. Use when the user asks to "add hooks", "handle parse/completion errors", "add retries/telemetry/logging", "register a hook", "instrument the agent", or runs `/atomic-agents:add-hooks`.
---
...
For the full reference (the five events, retry mechanics, production logging), the authority is `../framework/references/hooks.md`. This skill is the action-oriented path: pick events → write handlers → register → verify.
...
- **`framework` skill**: questions about Atomic Agents in general, or the user is doing something other than instrumenting an agent.
...
- Optional next steps:
  - Building or wiring the agent itself → `create-atomic-agent` skill.
...
For deeper material — the five events in full, retry mechanics, the repo's working example (`atomic-examples/hooks-example/hooks_example/main.py`) — load `../framework/references/hooks.md`.
