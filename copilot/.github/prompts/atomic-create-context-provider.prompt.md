...
For deep material (caching strategies, async data sources, multi-agent sharing patterns), the authority is `../references/context-providers.md`. This skill is the action-oriented path: clarify → write → register.
...
- **`/atomic-framework` command**: questions about Atomic Agents in general, or about something other than authoring a provider.
...
Then confirm the rendered system prompt includes the provider's section by inspecting `agent.system_prompt_generator.generate_prompt(...)` or by running the agent and checking the first request's payload via the `completion:kwargs` hook (see `../references/hooks.md`).
...
- Optional next steps:
  - The agent that consumes it → `/atomic-create-agent` command.
  - A research / RAG loop that updates the provider in a loop → see `atomic-examples/deep-research/` and `../references/orchestration.md`.
...
For deeper material — multi-agent sharing patterns, dynamic-update research loops, async source patterns — load `../references/context-providers.md`.
