...
When the caller did not specify, review unstaged changes by inspecting files already surfaced via `Read`. Do not run `git` yourself — the user provides scope.
...
Keep every issue short. Do not re-explain framework rules — the caller can reach into the `/atomic-framework` command's references for depth. Point there when a finding merits it:

- Schemas → `../references/schemas.md`
- Agents → `../references/agents.md`
- Tools → `../references/tools.md`
- Context providers → `../references/context-providers.md`
