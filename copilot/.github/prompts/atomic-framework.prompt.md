...
|---|---|
| "create a schema" / "design the input/output schema" | `/atomic-create-schema` |
| "create an agent" / "add another agent" / "wire up an `AtomicAgent`" | `/atomic-create-agent` |
| "add a tool" / "wrap an API as a tool" / "build a `BaseTool`" | `/atomic-create-tool` |
| "add a context provider" / "inject X into the prompt" / "wire up RAG" | `/atomic-create-context-provider` |
...
When a concept is unclear, start from the user's verb: *create a schema* → `/atomic-create-schema` command, *hook up a weather API* → `/atomic-create-tool` command, *inject user name into prompt* → `/atomic-create-context-provider` command, *route between agents* → orchestration reference.
...
Run the `/atomic-explore` command when the project has more than a handful of atomic-agents files and the user asks to "explore", "map", "understand how X works", or similar. It reads the relevant files and returns a compact architecture map (agents, tools, schemas, context providers, orchestration, essential-reading list). Run it with the scope (project root, module path, or feature) in the prompt.
...
For deeper guidance load the relevant reference file above. For code-review runs, delegate to the `/atomic-review` command.
