...
For deep material (validators, discriminated unions, error envelopes), the authority is `../references/schemas.md`. This skill is the action-oriented path: clarify → write → validate.
...
- **`/atomic-framework` command**: the user is asking about Atomic Agents in general or doing something other than authoring schemas.
...
4. **Failure modes** — can this legitimately fail? If yes, plan a typed error variant rather than raising. See `../references/schemas.md` → "Error-schema pattern".
...
- Next step in their flow:
  - Wiring it into an agent → `/atomic-create-agent` command.
  - Wiring it into a tool → `/atomic-create-tool` command.
  - Adding a context provider that depends on the same domain → `/atomic-create-context-provider` command.
...
For deeper material — composition, nested schemas, discriminated unions in production, validator gotchas — load `../references/schemas.md`.
