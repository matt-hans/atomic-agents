...
For deep material (search+execute over large tool surfaces, shared context providers, multi-agent memory), the authority is `../references/orchestration.md`. This skill is the action-oriented path: pick a pattern → wire types → run.
...
- **`/atomic-framework` command**: questions about Atomic Agents in general, or authoring a single agent (use `create-atomic-agent`).
