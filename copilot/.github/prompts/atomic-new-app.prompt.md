---
description: 'Scaffold a new Atomic Agents project from scratch — create the directory, `pyproject.toml`, env file, first agent, and a runnable entry point. Use when the user asks to start a new atomic-agents project from scratch, says "scaffold" / "new project" / "start from zero", or runs `/atomic-new-app`.'
mode: 'agent'
---
...
- Default model for the chosen provider (see `../references/providers.md`)
...
Use the template from `../references/project-structure.md`, substituting the chosen provider extra and project name.
...
Use the template from `../references/project-structure.md`.

### `<project_name>/main.py`

Produce a runnable REPL. Load `.env`, instantiate the provider client per `../references/providers.md`, build an agent, wire a `ChatHistory` with a seed assistant message, loop on `console.input(...)`.

For the agent itself, follow the workflow from the `/atomic-create-agent` skill — same canonical imports, same per-provider `mode` matrix, same `SystemPromptGenerator` shape.

When a custom agent type was requested, build custom `InputSchema` / `OutputSchema` subclasses with field `description=` populated, following the `/atomic-create-schema` skill. Otherwise use `BasicChatInputSchema` / `BasicChatOutputSchema`.
...
3. Next steps, picked from:
   - Replace the starter schemas with domain-specific ones — use the `/atomic-create-schema` skill.
   - Add another agent — use the `/atomic-create-agent` skill.
   - Add a tool — use the `/atomic-create-tool` skill.
   - Add a context provider (time, user, RAG, session) — use the `/atomic-create-context-provider` skill.
   - Split into multiple agents — see `../references/orchestration.md`.
...
- Never hand-roll what `../references/project-structure.md` already templates.
